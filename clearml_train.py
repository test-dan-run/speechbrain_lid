...
PROJECT_NAME = 'LID_Experiments'
TASK_NAME = 'vox_test'
OUTPUT_URL = 's3://why_am_i_so_cute:80/clearml-data/default'

task = Task.init(project=PROJECT_NAME, task_name=TASK_NAME, output_uri=OUTPUT_URL)
task.set_base_docker(docker_image='dleongsh/speechbrain:v0.5.11')
...
if __name__ == "__main__":
    ...
    dataset = Dataset.get(hparams_file['clearml_dataset_id'])
    dataset_task = Task.get_task(dataset.id)

    for split in ['train', 'dev', 'test']:
        hparams_file[f'{split}_annotation'] = dataset_task.artifacts[f'{split}_manifest'].get_local_copy()
    hparams_file['data_folder'] = dataset.get_local_copy()
